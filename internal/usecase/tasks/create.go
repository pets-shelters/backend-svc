...
func (uc *UseCase) Create(ctx context.Context, req requests.CreateTask, userId int64) error {
	...
	err = uc.repo.Transaction(ctx, func(tx pgx.Tx) error {
		taskId, err := uc.repo.GetTasksRepo().CreateWithConn(ctx, tx, entity.Task{
